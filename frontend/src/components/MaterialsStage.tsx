...
type Props = {
  ...
  productIdentity: { name: string; category: string; packageForm: string; sellingPoints: string; confirmed: boolean }
  onVideo: (file: File) => void
  onImage: (kind: AssetKind, file: File) => void
  onProductImages: (items: ProductImageUpload[], productName: string, productCategory: string, packageForm: string, sellingPoints: string) => Promise<boolean>
...
export function MaterialsStage(p: Props) {
  ...
  const videoCard = <AssetCard title="参考视频" required description="先在本地切分镜头；人工确认边界后才启动 Qwen 逐镜理解。" filename={p.video.filename} previewUrl={p.video.previewUrl} accept="video/mp4,video/quicktime,video/webm" uploading={p.video.uploading} onFile={p.onVideo} />
  return <section className="stage-content">
    ...
    <details className="optional-assets"><summary><span><strong>人物与背景参考</strong><small>仅在需要改变人物或背景时上传，AI 先转成文字档案</small></span><b>展开</b></summary><div className="material-grid">
      <AssetCard title="人物参考图或文字描述" description="有照片可以上传；文字只需填写年龄段、发型、主服装和显著特征，一两句话即可。" filename={p.materials.person.filename} previewUrl={p.materials.person.previewUrl} allowManualProfile profileHint="人物原图会随每个生成片段提交" deleting={p.deletingKind === 'person'} accept="image/jpeg,image/png,image/webp" status={p.materials.person.status} error={p.materials.person.error} profile={p.materials.person.profile} onFile={(file) => p.onImage('person', file)} onRetry={() => p.onRetry('person')} onProfile={(value) => p.onProfile('person', value)} onSaveProfile={() => p.onSaveProfile('person')} onDelete={() => p.onDelete('person')} />
      <AssetCard title="背景参考图" description="AI 提取空间、陈设、光线、景深与排除物。" filename={p.materials.background.filename} previewUrl={p.materials.background.previewUrl} profileHint="背景原图会随每个生成片段提交" deleting={p.deletingKind === 'background'} accept="image/jpeg,image/png,image/webp" status={p.materials.background.status} error={p.materials.background.error} profile={p.materials.background.profile} onFile={(file) => p.onImage('background', file)} onRetry={() => p.onRetry('background')} onProfile={(value) => p.onProfile('background', value)} onSaveProfile={() => p.onSaveProfile('background')} onDelete={() => p.onDelete('background')} />
    </div></details>
    <footer className="stage-footer"><div>{!p.ready && <p className="blocking-message">{p.blockingReason}</p>}<small>下一步只在本地切分视频，不会启动 Qwen。</small></div><button className="button primary large" disabled={!p.ready} onClick={p.onContinue}>开始切分视频</button></footer>
  </section>
}
